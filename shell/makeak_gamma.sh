...
datadir=../gpfs/data/skimmed/pandora/gamma

last="000_reco_REC"

a=0
for file in `cat filelists/gamma.list`; do
  echo $file
  filename_d=${file%.*}
  filename=${filename_d##*/}
  # name=${filename%${last}}
  ...
  # jobdir=job/skimmed/pandora/gamma/${filename}
  # mkdir -p ${jobdir}
  mkdir -p ${datadir}/log/concat
  mkdir -p ${datadir}/awkd/concat
  jobdir=job/skimmed/pandora/gamma/concat
  mkdir -p ${jobdir}
...
<< COMMENTOUT
  for i in `seq 0 99`; do
    ...
    echo bsub -q s -o ${jobdir}/output.%J -e ${jobdir}/errors.%J "python LCIO2ak2_single_particle.py ${file} ${datadir}/awkd/${filename}/${filename}_${S}.h5 5 ${a} > ${datadir}/log/${filename}/${filename}_${S}.log"
