...
datadir=../gpfs/data/skimmed/pandora/kaon

last="000_reco_REC"

a=0
for file in `cat filelists/kaon.list`; do
  echo $file
  filename_d=${file%.*}
  filename=${filename_d##*/}
  # name=${filename%${last}}
  ...
  # jobdir=job/skimmed/pandora/kaon/${filename}
  # mkdir -p ${jobdir}
  mkdir -p ${datadir}/log/concat
  mkdir -p ${datadir}/awkd/concat
  jobdir=job/skimmed/pandora/kaon/concat
  mkdir -p ${jobdir}
