...
mkdir -p job/skimmed/pandora/${type}/concat


for file in `cat filelists/ntau_10GeV_10_${sample}Samples.txt`; do
    # echo $file
    filename_d=${file%.*}
    filename=${filename_d##*/}
    # name=${filename%${last}}
    ...
    bsub -q s -o job/skimmed/pandora/${type}/concat/output.%J -e job/skimmed/pandora/${type}/concat/errors.%J "python concat_awkward.py temp/temp.list ${basedir}/concat/${type}_${num}.h5"
