...
for file in `cat filelists/uds91.list`; do
  ...
  mkdir -p job/skimmed/uds91/${filename}
  ...
  for i in `seq 0 99`; do
    ...
    bsub -q s -o job/skimmed/uds91/${filename}/output.%J -e job/skimmed/uds91/${filename}/errors.%J "python LCIO2ak2_edit_skimmed.py $file ${datadir}/awkd/${filename}/${filename}_${S}.h5 5 ${a} > ${datadir}/log/${filename}/${filename}_${S}.log"
    # echo "finish"
  done
  # sleep 100
done
