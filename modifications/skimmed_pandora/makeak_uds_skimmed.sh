...
datadir=../gpfs/data/skimmed/pandora/uds91
...
for file in `cat filelists/uds91.list`; do
  ...
  jobdir=job/skimmed/pandora/uds91/${filename}
  mkdir -p ${jobdir}
  ...
  for i in `seq 0 99`; do
    ...
    bsub -q s -o ${jobdir}/output.%J -e ${jobdir}/errors.%J "python LCIO2ak2_edit_skimmed_pandora.py ${file} ${datadir}/awkd/${filename}/${filename}_${S}.h5 5 ${a} > ${datadir}/log/${filename}/${filename}_${S}.log"
