...
def makeAk(filename, outfilename, maxread, skip):
    ...
    for idx,event in enumerate(reader):
        ...
        for colname in PandoraList:
            ...
            col = event.getCollection(colname)
            for hit in col:
                # print("Pandora ", hit, hit.id(), hit.getEnergy(), hit.getParticles(), hit.getClusters().size())
        ...
        for colname in PandoraList:
            ...
            for hit in col:
                ...
                for cluster in hit.getClusters():
                    # print("    cluster : ", cluster.id())
                    for calhit in cluster.getCalorimeterHits():
                        # print("        calhit : ", calhit.id())
                        calhitid = calhit.id()
                        if not calhitid in dictPandora.keys():
                            #isDeltaRay(mc)
                            dictPandora[calhitid] = {"id":len(dictPandora), "clusterId":cluster.id(), "energy":hit.getEnergy(), "pdg":hit.getParticleIDs(), "charge":hit.getCharge(), "mass":hit.getMass(), "momentum":hit.getMomentum()}#, "status":hit.getSimulatorStatus()}

                        for relcolname in RelHitList:       ## これでpandoraHitとsimHitの対応はできている
                            if not relcolname in colnames: continue
                            relcol = event.getCollection(relcolname)
                            nav = LCRelationNavigator(relcol)
                            for rel,w in zip(nav.getRelatedToObjects(calhit),nav.getRelatedToWeights(calhit)):
                                # print("            ", rel.id(), w)
                                if w > 0.5:
                                    if w < 1: print (w)
                                    mcid = dictSimMc[rel.id()]
                                    ...
                                    if not mcid in dictPandora.keys():
                                        #isDeltaRay(mc)
                                        dictPandora[mcid] = {"id":len(dictPandora),"energy":hit.getEnergy(), "pdg":hit.getParticleIDs(), "charge":hit.getCharge(), "mass":hit.getMass(), "momentum":hit.getMomentum()}#, "status":hit.getSimulatorStatus()}

                                        #if mc.getPDG() == 22 and mc.getParents().size() > 0 and mc.getParents()[0].getCharge() != 0:
                                        #    pmc = mc.getParents()[0]
                                        #    print("Photon with parent",pmc.getPDG(), "MC_ID", dictClusterIdx[mcid]["id"], "PMC_ID", pmc.id(), "photon momentum", mc.getMomentum()[0], mc.getMomentum()[1], mc.getMomentum()[2])

                b_pandora_label.real(hitid)
                if(hitid != 0):
                    b_pandora_label.real(labels["id"])
                    b_pandora_label.real(labels["pdg"])
                    b_pandora_label.real(labels["charge"])
                    b_pandora_label.real(labels["mass"])
                    b_pandora_label.real(labels["momentum"][0])
                    b_pandora_label.real(labels["momentum"][1])
                    b_pandora_label.real(labels["momentum"][2])
                    b_pandora_label.real(labels["status"])
                else:
                    b_pandora_label.real(-1) #labels["id"])
                    b_pandora_label.real(0) #labels["pdg"])
                    b_pandora_label.real(0) #labels["charge"])
                    b_pandora_label.real(0) #labels["mass"])
                    b_pandora_label.real(0) #labels["momentum"][0])
                    b_pandora_label.real(0) #labels["momentum"][1])
                    b_pandora_label.real(0) #labels["momentum"][2])
                    b_pandora_label.real(0) #labels["status"])
                #print ("labels finished.")
                b_pandora_label.end_list()


                # for relhitname in RelPandoraList:
                #     if not relhitname in colnames: continue
                #     relcol = event.getCollection(relhitname)
                #     nav = LCRelationNavigator(relcol)
                #     for rel,w in zip(nav.getRelatedToObjects(hit),nav.getRelatedToWeights(hit)):
                #         if w > 0.5:
                #             if w < 1: print (w)
                ...
                # for relhitname in RelPandoraList:
                #     if not relhitname in colnames: continue
                #     relhit = event.getCollection(relhitname)
                #     nav = LCRelationNavigator(relhit)
                #     # for nmc in range(simhit.getNMCContributions()):
                ...
                #             hitid = hit.id()
        if havehit == 0:
            print("No Pandora hit in the event ", idx)
            continue
